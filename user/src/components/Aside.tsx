...
const Aside = () => {
    ...
    return (
        ...
        animate={{ width: isAsideOpen ? '160px' : '90px' }}
        ...
        <div className="h-1/2 flex flex-col justify-end items-start pb-10 gap-7 relative">
            <span className="flex items-center w-full pl-7 gap-2">
                ...
                <motion.p 
                    ...
                    animate={{ opacity: isAsideOpen ? 1 : 0, width: isAsideOpen ? 'auto' : 0 }}
                    transition={{ duration: 0.3, ease: 'easeInOut' }}
                >
                    Settings
                </motion.p>
            ...
            <span className="flex items-center w-full pl-7 gap-2">
                <Menu onClick={handleAsideOpen} color="white" className="w-8 h-8" cursor="pointer"/>
                <motion.p 
                    className="text-white font-semibold text-[1.2rem] overflow-hidden"
                    initial={{ opacity: 0, width: 0 }}
                    animate={{ opacity: isAsideOpen ? 1 : 0, width: isAsideOpen ? 'auto' : 0 }}
